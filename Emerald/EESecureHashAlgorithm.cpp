...
namespace Emerald
{
	...
	std::string EESecureHashAlgorithm::SHA_1(const std::string& _input)
	{
		...
		unsigned int k[4] = { 0x5a827999, 0x6ed9eba1, 0x8f1bbcdc, 0xca62c1d6 }; //  230 times the square roots of 2, 3, 5 and 10 (Nothing up my sleeve number)
		unsigned int H[5] = { 0x67452301, 0xEFCDAB89, 0x98BADCFE, 0x10325476, 0xC3D2E1F0 }; //  Same with the MD5 algorithm
		for (std::array<unsigned char, 64>& chunk : chunks)
		{
			...
			for (int i = 0; i < 80; ++i)
			{
				unsigned int f;
				if (i <= 19)
					f = ((h[1] & h[2]) | ((~h[1]) & h[3])) + k[0];
				else if (i <= 39)
					f = (h[1] ^ h[2] ^ h[3]) + k[1];
				else if (i <= 59)
					f = ((h[1] & h[2]) | (h[1] & h[3]) | (h[2] & h[3])) + k[2];
				else
					f = (h[1] ^ h[2] ^ h[3]) + k[3];
	...
	std::string EESecureHashAlgorithm::SHA_2_384(const std::string& _input)
	{
		// Preprocess
		...
		unsigned long long int H[8] = { 0xcbbb9d5dc1059ed8, 0x629a292a367cd507, 0x9159015a3070dd17, 0x152fecd8f70e5939, 0x67332667ffc00b31, 0x8eb44a8768581511, 0xdb0c2e0d64f98fa7, 0x47b5481dbefa4fa4 };
		...
		std::ostringstream stringStream;
		for (int i = 0; i < 6; ++i) {
			stringStream << std::hex << std::setfill('0') << std::setw(16) << H[i];
		}
	...
	std::string EESecureHashAlgorithm::SHA_2_512(const std::string& _input)
	{
		...
		std::ostringstream stringStream;
		for (auto h : H) {
			stringStream << std::hex << std::setfill('0') << std::setw(16) << h;
		}
	...
	//----------------------------------------------------------------------------------------------------
	std::string EESecureHashAlgorithm::SHA_2_512_224(const std::string& _input)
	{
		// Preprocess
		std::vector<std::array<unsigned char, 128>> chunks;
		Preprocess(_input, chunks);
		...
		const unsigned long long int k[80] = {
			...
			0x113f9804bef90dae, 0x1b710b35131c471b, 0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc,
			0x431d67c49c100d4c, 0x4cc5d4becb3e42b6, 0x597f299cfc657e2a, 0x5fcb6fab3ad6faec, 0x6c44198c4a475817 };
		unsigned long long int H[8] = { 0x8C3D37C819544DA2, 0x73E1996689DCD4D6, 0x1DFAB7AE32FF9C82, 0x679DD514582F9FCF, 0x0F6D2B697BD44DA8, 0x77E36F7304C48942, 0x3F9D85A86A1D36C8, 0x1112E6AD91D692A1 };
		for (std::array<unsigned char, 128>& chunk : chunks)
		{
			// Break chunk into sixteen 64-bit big-endian words
			std::vector<unsigned long long int> words;
		...
		stringStream << std::hex << std::setfill('0') << std::setw(8) << (unsigned int)(H[3] >> 32);

		return stringStream.str();
	}

	//----------------------------------------------------------------------------------------------------
	std::string EESecureHashAlgorithm::SHA_2_512_256(const std::string& _input)
	{
		// Preprocess
		std::vector<std::array<unsigned char, 128>> chunks;
		Preprocess(_input, chunks);
		...
		const unsigned long long int k[80] = {
			...
			0x113f9804bef90dae, 0x1b710b35131c471b, 0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc,
			0x431d67c49c100d4c, 0x4cc5d4becb3e42b6, 0x597f299cfc657e2a, 0x5fcb6fab3ad6faec, 0x6c44198c4a475817 };
		unsigned long long int H[8] = { 0x22312194FC2BF72C, 0x9F555FA3C84C64C2, 0x2393B86B6F53B151, 0x963877195940EABD, 0x96283EE2A88EFFE3, 0xBE5E1E2553863992, 0x2B0199FC2C85B8AA, 0x0EB72DDC81C52CA2 };
		for (std::array<unsigned char, 128>& chunk : chunks)
		{
			// Break chunk into sixteen 64-bit big-endian words
			std::vector<unsigned long long int> words;
